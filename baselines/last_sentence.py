...
DATA_PATH = "../data/"
RESULT_PATH = "../data/baseline_results/last_sentence/"
...
def _get_last_sentence(text):
    if text != "":
        if text[0] == ".":
            text = text[1:].strip()
        sentences = nltk.tokenize.sent_tokenize(text.strip(), language='english')
        if len(sentences) > 0:
            return sentences[-1]
    return "-"

def compute(entries):
    return [{"id": entry["id"], "answer": _get_last_sentence(entry["text"])} for entry in entries]
